...
class DNILMF:
    ...
    def fix_model(self, W, intMat, drugMat, targetMat,num, cvs, dataset, seed=None):
        ...
        self.cvs = cvs
        self.seed = seed
        np.savetxt('DNILMFW.txt', W)
        command = 'Rscript'
        path2script = 'dnilmf.R'
        args=[self.dataset]
        cmd = [command, path2script] + args
        subprocess.call(cmd,universal_newlines=False)
        #self.predictR =  open('DNILMFscore.txt','w')
    ...
    def evaluation(self, test_data, test_label):
        with open("DNILMFscore.txt", "r") as inf:
            inf.readline()
        ...
        scores.to_csv('../data/datasets/EnsambleDTI/dnilmf_'+str(self.dataset)+'_s'+

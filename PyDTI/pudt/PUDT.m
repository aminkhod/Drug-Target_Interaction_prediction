...
% division = repmat(1:2,1,ceil(na*nb/1)); 
%for rep = 1:opts.num_repetitions
% 		which_fold{rep} = division(randperm(na*nb));   
% 		which_fold{1} = division(randperm(na*nb));   
%end
	% Run the tests
...
%             Ptest = setdiff(test',Utest);
            ...
            for ii=1:length(Record)
                ...
                [p,~,v] = svmpredict(ones(1,length(indTest1))',[(1:length(indTest1))',K_Test'],model); 
                mypre = v.*sign(p - 1/2);
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%RL
                 lable(lable==0)=-1;
                ...
                [p1,~,v1] = svmpredict(ones(1,length(indTest1))',[(1:length(indTest1))',K_Test'],model1); 
